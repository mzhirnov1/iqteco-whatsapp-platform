...
## LID: сломанная сессия — не «номера нет в WhatsApp»
 ...
 резолвит адрес перед отправкой: сначала ,
затем — если не вышло — чат по . Оба вызова раньше глотали
исключение, и ответ был один: . Но когда WA Web Store
разваливается (та же поломка, из-за которой падает ), бросают **все**
...
- Теперь бросок отличается от честного «не нашли»: если ни один lookup не
  ответил, шлём на  и даём  сообщить настоящую ошибку.
   остаётся, только когда оба ответили и ничего не нашли.
- Тест —  (в том числе сессия, у которой всё бросает).
- Номер LID-чата платформа отдаёт потребителям в 
  (, кеш на 5000 пар) — читать надо именно его,  там
  внутренний идентификатор. Массовый резолв — 
